...
class FactorOracle:
			...
			if cf < continuity_factor_min and current_state.exiting_transition != None and (clock+current_state.exiting_transition.content.duration)<scenario[ind_scenario+1][1] or n==0:
				...
				for trans in attainable_trans:
					if not(trans.end in taboo_list):
						attainable_sans_taboo.append(trans)
					if attainable_sans_taboo != []:
						selected_trans = self.transitionChoice(current_state, attainable_sans_taboo, proba_model, scenario, ind_scenario)
					else:
						selected_trans = self.transitionChoice(current_state, attainable_trans, proba_model, scenario, ind_scenario)
					if selected_trans == current_state.exiting_transition:
						cf = cf+1
					else:
						cf = 0
	...
	def transitionChoice(self, current_state, transition_list, proba_model, scenario, ind_scenario):
		...
		prev_note_count = 0
		for count in bigram.counts:
			if count.condition == prev_note:
				prev_note_count = prev_note_count + count.count
		
		chord_count = 0
		for count in melody_chord.counts:
			if count.condition == chord:
				chord_count = chord_count + count.count
		...
		for t in range(len(transition_list)):
			...
			bigram_count = 0
			i = 0
			while found == False and i < len(bigram.counts):
				bc = bigram.counts[i]
				if bc.event == note and bc.condition == prev_note:
					found == True
					bigram_count = bc.count
				i = i+1
			if prev_note_count == 0:
				bigram_proba = 0
			else:
				bigram_proba = (bigram_count / (1.0*prev_note_count))

			found = False
			melody_chord_count = 0
			i = 0
			while found == False and i < len(melody_chord.counts):
				mcc = melody_chord.counts[i]
				if mcc.event == note and mcc.condition == chord:
					found == True
					melody_chord_count = mcc.count
				i = i+1
			if chord_count == 0:
				melody_chord_proba = 0
			else:
				melody_chord_proba = (melody_chord_count / (1.0*chord_count))

			transition_proba[t] = proba_model.coefficients[0]*bigram_proba + proba_model.coefficients[1]*melody_chord_proba + proba_model.add_smooth_coef
		...
		temp_sum = 0
		while choice == False:
			if rand < temp_sum + transition_proba[i]:
				choice = True
			else:
